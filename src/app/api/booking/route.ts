...
export async function POST(request: NextRequest) {
  try {
    ...
    const createdBooking = await createBookingWithConflictCheck(processedBookingData);
        ...
        return NextResponse.json(
          {
            ...
            message: "The selected time slot was booked by another user. Please select a different time.",
 ...
 */
async function validateBookingSubmission(data: BookingSubmission): Promise<BookingValidationResult> {
  const errors: string[] = [];
  ...
  try {
    ...
    if (data.preferredDate && data.preferredTime) {
      ...
      if (!calendarValidation.valid) {
        errors.push(calendarValidation.reason || "Selected time slot is invalid");
      }
    }

    // Data sanitization warnings
    if (data.name && data.name !== JavaScriptSafetyUtils.sanitizeString(data.name)) {
      warnings.push("Name contains potentially unsafe characters and will be cleaned");
    }
...
async function verifyBookingAvailability(data: BookingSubmission): Promise<{
  ...
  try {
    ...
    const requestedDateTime = new Date(`${data.preferredDate}T${data.preferredTime}:00`);
    ...
    const isBooked = existingBookings.bookings.some(booking => {
      ...
      return bookingDate.toDateString() === requestedDateTime.toDateString() && 
    ...
    if (isBooked) {
      ...
      return {
        ...
        alternatives: alternativeSlots.map(slot => `${slot.date} ${slot.time}`)
      };
    }
...
async function processBookingData(data: BookingSubmission) {
  return {
    ...
    customerRequest: data.requirements || `${data.bookingType || 'demo'} booking request`,
    originalMessage: data.requirements || `Calendar booking for ${data.bookingType || 'demo'}`,
    ...
    email: JavaScriptSafetyUtils.sanitizeString(data.email.toLowerCase().trim()),
    company: data.company ? JavaScriptSafetyUtils.sanitizeString(data.company) : undefined,
    phone: data.phone ? JavaScriptSafetyUtils.sanitizeString(data.phone) : undefined,
    role: data.role,
    
    // Booking details
    requestType: data.bookingType || "demo",
    requirements: data.requirements ? JavaScriptSafetyUtils.sanitizeString(data.requirements) : undefined,
 ...
 */
async function sendBookingConfirmation(booking: any, originalData: BookingSubmission) {
  try {
    ...
    console.log("📧 Booking confirmation (email service not implemented):", {
      to: booking.email,
      confirmationNumber: booking.confirmationNumber,
      scheduledFor: `${booking.preferredDate.toDateString()} at ${booking.preferredTime}`,
      bookingType: booking.requestType
    });
...
function generateNextSteps(booking: any): string[] {
  const steps = [
    ...
    `📅 Add the event to your calendar for ${booking.preferredDate.toDateString()} at ${booking.preferredTime}`,
    "📋 Prepare any specific questions you'd like to discuss",
  ];

  if (booking.requestType === "demo") {
    steps.push("💻 Ensure you have a stable internet connection for the video call");
  }

  if (booking.phone) {
    steps.push("📞 We'll call you at the scheduled time if video isn't working");
  }
