...
//   );
// }
// Advancelytics — Customer Support AI (Full Page, Live Preview)
// ✅ Updates added per request:
// - CTA urgency: "Start Free — Boost CSAT Today"
// - Trial counter microcopy near CTA: "14-day trial — no card needed (start in 60 sec)"
// - StoryBrand 3-step plan: Connect → Learn → Automate
// - Hero badges row: Zendesk, Freshdesk, Intercom, Salesforce
// - Testimonials: Auto-rotating logo & quote carousel (Framer Motion)
// - Meta keywords (commented for SPA preview): proactive CX platform, AI support automation, AI deflection tools
        ...
        <div className="mx-auto grid max-w-7xl items-center gap-12 md:grid-cols-2">
          <div>
            ...
            <div className="mt-8 flex flex-wrap items-center gap-3">
              ...
              <span className="ml-1 inline-flex items-center gap-2 rounded-full border border-[--border-subtle] bg-white px-3 py-1 text-xs font-medium text-slate-600">
                <span className="inline-block h-2 w-2 animate-pulse rounded-full bg-emerald-500" />
                14-day trial — no card needed (start in 60 sec)
              </span>
            </div>

            {/* Badges row for integrations */}
            <div
              className="mt-5 flex flex-wrap items-center gap-2 opacity-90"
              aria-label="Works with"
            >
              {["Zendesk", "Freshdesk", "Intercom", "Salesforce"].map((b) => (
                <span
                  key={b}
                  className="rounded-full border border-[--border-subtle] bg-white px-3 py-1.5 text-xs font-medium text-slate-600"
                >
                  {b}
                </span>
              ))}
      ...
      >
        ...
        <div className="mt-8 grid gap-6 sm:grid-cols-3">
          {[
            ...
            <div
              key={i}
              className="group relative overflow-hidden rounded-2xl border border-[--border-subtle] bg-white p-6 text-center shadow-sm transition hover:shadow-md"
            >
              <div
                className="pointer-events-none absolute inset-px rounded-2xl opacity-0 blur-xl transition-opacity duration-300 group-hover:opacity-100"
                style={{
              ...
              <div className="relative z-10">
                <div className="mx-auto grid h-12 w-12 place-items-center rounded-2xl bg-[--brand-primary]/10 text-2xl">
                  {s.icon}
                </div>
              ...
              <div className="relative z-10 mx-auto mt-4 h-1 w-0 rounded bg-[--brand-primary] transition-all duration-500 group-hover:w-24" />
            </div>
          ))}
        </div>
      </section>
            ...
            <AnimatePresence mode="popLayout">
              ...
              >
                {Array.from({ length: 3 }).map((_, col) => {
                  const t =
                    testimonials[(activeIndex + col) % testimonials.length];
                  return (
                    <figure
                      key={col}
                      className="group relative overflow-hidden rounded-2xl border border-[--border-subtle] bg-white p-6 shadow-sm transition hover:shadow-md"
            ...
            {testimonials.map((_, i) => (
              <button
                ...
                className={`h-2.5 w-2.5 rounded-full ${
                  i === activeIndex ? "bg-[--brand-primary]" : "bg-slate-300"
                }`}
              />
            ))}
          </div>
