...
export async function autoGenerateDiagnosticAnswers(
  ...
  try {
    ...
    const results = new Map<string, { answer: string; options: string[] }>(); // key -> { answer, options }
    ...
    const processOption = async (opt: { label: string; workflow: string }) => {
      try {
        ...
        if (!result.options || result.options.length === 0) {
          console.warn(
            `[Diagnostic] Warning: No options generated for ${opt.label}. Using fallback options.`,
          );
          result.options = [
            "View Details",
            "Contact Sales",
...
export async function generateSingleDiagnosticAnswer(
  ...
  workflow: string,
  context: string,
): Promise<{ answer: string; options: string[] }> {
  try {
    const prompt = `
SYSTEM:
...
`;
    ...
    return {
      ...
      options: Array.isArray(data.diagnostic_options)
