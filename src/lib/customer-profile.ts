...
export function computeBant(profile: any) {
...
export interface UpdateCustomerProfileParams {
  ...
  sessionId: string;
  email?: string;
  conversation?: string | { role: string; content: string }[];
  messageCount?: number;
  timeInSession?: number;
  pageTransitions?: string[];
  pageUrl?: string;
...
export async function updateCustomerProfile({
  ...
  try {
    ...
    // Calculate engagement metrics from DB (Single Source of Truth)
    let calculatedMessageCount = 0;
    let calculatedDurationSec = 0;

    try {
      const allSessionIds = existingProfile
        ? [...new Set([...(existingProfile.sessionIds || []), sessionId])]
        : [sessionId];

      const stats = await chats
        .aggregate([
      ...
      calculatedMessageCount = stats.reduce((acc, s) => acc + s.msgCount, 0);
      const totalDurationMs = stats.reduce(
        (acc, s) => acc + (s.duration || 0),
        0,
      );
      calculatedDurationSec = Math.floor(totalDurationMs / 1000);
    } catch (statsError) {
      console.warn("Failed to calculate session stats:", statsError);
    }
    ...
    const effectiveMessageCount = messageCount ?? calculatedMessageCount;
    const effectiveTimeInSession = timeInSession ?? calculatedDurationSec;

    const updateDecision = shouldUpdateProfile(
      effectiveMessageCount,
      Array.isArray(conversation)
        ? conversation.map((msg: any) => msg.content || "")
        : [conversationContent],
      effectiveTimeInSession,
      pageTransitions || [],
    ...
    const updatedProfile = {
      ...
      engagementProfile: {
        ...
        questionsAsked:
          effectiveMessageCount || baseProfile.engagementProfile.questionsAsked,
        pagesVisited: pageUrl
          ...
          : baseProfile.engagementProfile.pagesVisited,
        timeOnSite:
          effectiveTimeInSession || baseProfile.engagementProfile.timeOnSite,
        returnVisits: (baseProfile.sessionIds || []).includes(sessionId)
          ? baseProfile.engagementProfile.returnVisits
          : (baseProfile.engagementProfile.returnVisits || 0) + 1,
      },
      intelligenceProfile: {
