...
db.sdr_events.find({event_type: "cross_page_persistence"}).sort({timestamp: -1})
...
db.sdr_events.aggregate([
  ...
  {$group: {
    _id: null,
    total_emails: {$sum: 1},
    with_personas: {$sum: {$cond: [{$gt: [{$size: "$persona"}, 0]}, 1, 0]}}
  }},
  {$project: {
    total_emails: 1,
    with_personas: 1,
    conversion_rate: {$multiply: [{$divide: ["$with_personas", "$total_emails"]}, 100]}
  }}
])
...
db.sdr_events.aggregate([
  ...
  {$group: {
    _id: "$vertical_detected",
    email_captures: {$sum: {$cond: [{$eq: ["$event_type", "email_captured"]}, 1, 0]}},
    total_events: {$sum: 1},
  ...
  {$project: {
    ...
    conversion_rate: {$multiply: [{$divide: ["$email_captures", "$total_events"]}, 100]}
  }},
